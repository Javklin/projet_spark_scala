...
def check_content(df: org.apache.spark.sql.DataFrame ) :Boolean = {
    ...
    //nb de mot
    val dfWithWordCount = df.withColumn("word_count", size(array_remove(split(col("book"), "\\s+"), "")))
    dfWithWordCount.show(false)
    val totalWordCount = dfWithWordCount.agg(sum("word_count")).collect()(0)(0)
    println(s"Total number of words: $totalWordCount *******************************************************************************")
    ...
    dfWithSentenceCount.show(false)
